//altlvds_rx CBX_DECLARE_ALL_CONNECTED_PORTS="OFF" COMMON_RX_TX_PLL="OFF" CYCLONEII_M4K_COMPATIBILITY="ON" DESERIALIZATION_FACTOR=16 DEVICE_FAMILY="Stratix II" IMPLEMENT_IN_LES="ON" INCLOCK_PERIOD=32000 INCLOCK_PHASE_SHIFT=0 INPUT_DATA_RATE=500 NUMBER_OF_CHANNELS=1 OUTCLOCK_RESOURCE="AUTO" PLL_OPERATION_MODE="NORMAL" PORT_RX_DATA_ALIGN="PORT_USED" REGISTERED_DATA_ALIGN_INPUT="ON" REGISTERED_OUTPUT="ON" USE_EXTERNAL_PLL="OFF" rx_data_align rx_in rx_inclock rx_locked rx_out rx_outclock CARRY_CHAIN="IGNORE" CARRY_CHAIN_LENGTH=48 LOW_POWER_MODE="AUTO" ALTERA_INTERNAL_OPTIONS=AUTO_SHIFT_REGISTER_RECOGNITION=OFF
...
//CBXI_INSTANCE_NAME="sigma_delta_CL_Rx_inst6_CL_lvds_rx1_inst55_altlvds_rx_altlvds_rx_component"
...
`endif
	...
	assign
		...
		inclk_wire = {inclk},
		outclk = wire_clkctrl7_outclk;
endmodule //CL_lvds_rx1_altclkctrl
...
endmodule //CL_lvds_rx1_ddio_in


//dffpipe CBX_DECLARE_ALL_CONNECTED_PORTS="OFF" DELAY=6 WIDTH=1 clock d q ALTERA_INTERNAL_OPTIONS=AUTO_SHIFT_REGISTER_RECOGNITION=OFF
...
`endif
	...
	always @ ( posedge clock or  negedge prn or  negedge clrn)
		...
		else if  (ena == 1'b1)   dffe12a <= (dffe11a & (~ sclr));
	// synopsys translate_off
	initial
		dffe13a = 0;
	// synopsys translate_on
	...
	always @ ( posedge clock or  negedge prn or  negedge clrn)
		...
		else if  (ena == 1'b1)   dffe9a <= (dffe8a & (~ sclr));
	assign
		clrn = 1'b1,
		ena = 1'b1,
		prn = 1'b1,
		q = dffe13a,
		sclr = 1'b0;
endmodule //CL_lvds_rx1_dffpipe


//lpm_counter CBX_DECLARE_ALL_CONNECTED_PORTS="OFF" DEVICE_FAMILY="Stratix II" lpm_modulus=16 lpm_port_updown="PORT_UNUSED" lpm_width=4 clock cnt_en q
...
`endif
	...
	wire  [3:0]   wire_counter_reg_bit14a_adatasdata;
	wire  [3:0]   wire_counter_reg_bit14a_regout;
	wire  aclr_actual;
	wire clk_en;
	wire [3:0]  data;
	wire  external_cin;
	...
	.datad(wire_counter_reg_bit14a_regout[0:0]),
	.dataf(updown_lsb),
	.shareout(),
	...
	.datad(wire_counter_reg_bit14a_regout[1:1]),
	...
	.datad(wire_counter_reg_bit14a_regout[2:2]),
	.dataf(updown_other_bits),
	.shareout(),
	...
	.combout(),
	.cout(),
	.datad(wire_counter_reg_bit14a_regout[3:3]),
	.dataf(updown_other_bits),
	.shareout(),
	...
	stratixii_lcell_ff   counter_reg_bit14a_0
	( 
	.aclr(aclr_actual),
	.adatasdata(wire_counter_reg_bit14a_adatasdata[0:0]),
	.clk(clock),
	.datain(wire_counter_comb_bita_0sumout[0:0]),
	.ena((clk_en & (((cnt_en | sclr) | sset) | sload))),
	.regout(wire_counter_reg_bit14a_regout[0:0]),
	.sclr(sclr),
	.sload((sset | sload)),
	.aload(1'b0)
	// synopsys translate_off
	...
	stratixii_lcell_ff   counter_reg_bit14a_1
	( 
	.aclr(aclr_actual),
	.adatasdata(wire_counter_reg_bit14a_adatasdata[1:1]),
	.clk(clock),
	.datain(wire_counter_comb_bita_1sumout[0:0]),
	.ena((clk_en & (((cnt_en | sclr) | sset) | sload))),
	.regout(wire_counter_reg_bit14a_regout[1:1]),
	.sclr(sclr),
	.sload((sset | sload)),
	.aload(1'b0)
	// synopsys translate_off
	...
	stratixii_lcell_ff   counter_reg_bit14a_2
	( 
	.aclr(aclr_actual),
	.adatasdata(wire_counter_reg_bit14a_adatasdata[2:2]),
	.clk(clock),
	.datain(wire_counter_comb_bita_2sumout[0:0]),
	.ena((clk_en & (((cnt_en | sclr) | sset) | sload))),
	.regout(wire_counter_reg_bit14a_regout[2:2]),
	.sclr(sclr),
	.sload((sset | sload)),
	.aload(1'b0)
	// synopsys translate_off
	...
	stratixii_lcell_ff   counter_reg_bit14a_3
	( 
	.aclr(aclr_actual),
	.adatasdata(wire_counter_reg_bit14a_adatasdata[3:3]),
	.clk(clock),
	.datain(wire_counter_comb_bita_3sumout[0:0]),
	.ena((clk_en & (((cnt_en | sclr) | sset) | sload))),
	.regout(wire_counter_reg_bit14a_regout[3:3]),
	.sclr(sclr),
	.sload((sset | sload)),
	.aload(1'b0)
	// synopsys translate_off
	...
	assign
		wire_counter_reg_bit14a_adatasdata = (({4{sset}} & s_val) | ({4{(~ sset)}} & data));
	assign
		aclr_actual = 1'b0,
		clk_en = 1'b1,
		data = {4{1'b0}},
		...
		q = safe_q,
		s_val = {4{1'b1}},
		safe_q = wire_counter_reg_bit14a_regout,
		sclr = 1'b0,
		sload = 1'b0,
		sset = 1'b0,
		updown_dir = 1'b1,
...
endmodule //CL_lvds_rx1_cntr


//lpm_mux CBX_DECLARE_ALL_CONNECTED_PORTS="OFF" DEVICE_FAMILY="Stratix II" LPM_SIZE=16 LPM_WIDTH=1 LPM_WIDTHS=4 data result sel
...
endmodule //CL_lvds_rx1_mux

//synthesis_resources = clkctrl 1 lut 14 reg 65 stratixii_io 1 stratixii_pll 1 
//synopsys translate_off
`timescale 1 ps / 1 ps
//synopsys translate_on
(* ALTERA_ATTRIBUTE = {"AUTO_SHIFT_REGISTER_RECOGNITION=OFF;{-to lvds_rx_pll} AUTO_MERGE_PLLS=OFF"} *)
module  CL_lvds_rx1_lvds_rx
	( 
	rx_data_align,
	rx_in,
	rx_inclock,
...
`endif
	...
	reg	[6:0]	cda_h_shiftreg3a;
	reg	[7:0]	cda_l_shiftreg4a;
	reg	[7:0]	h_shiftreg2a;
	reg	int_bitslip_reg;
	reg	[7:0]	l_shiftreg1a;
	...
	always @ ( posedge fast_clock)
		  cda_h_shiftreg3a <= {cda_h_shiftreg3a[5:0], ddio_dataout_h[0]};
	// synopsys translate_off
	initial
		cda_l_shiftreg4a = 0;
	// synopsys translate_on
	always @ ( posedge fast_clock)
		  cda_l_shiftreg4a <= {cda_l_shiftreg4a[6:0], ddio_dataout_l[0]};
	// synopsys translate_off
	initial
		h_shiftreg2a = 0;
	// synopsys translate_on
	always @ ( posedge fast_clock)
		  h_shiftreg2a <= {h_shiftreg2a[6:0], wire_l_mux6a_result};
	// synopsys translate_off
	initial
		int_bitslip_reg = 0;
	// synopsys translate_on
	...
	// synopsys translate_on
	always @ ( posedge fast_clock)
		  l_shiftreg1a <= {l_shiftreg1a[6:0], wire_h_mux5a_result};
	// synopsys translate_off
	initial
		rx_data_align_reg = 0;
	// synopsys translate_on
	...
	always @ ( posedge slow_clock)
		  rx_reg <= rx_out_wire;
	CL_lvds_rx1_dffpipe   h_dffpipe
	( 
	.clock(fast_clock),
	.d(ddio_dataout_h_int),
	.q(wire_h_dffpipe_q));
	CL_lvds_rx1_dffpipe   l_dffpipe
	( 
	.clock(fast_clock),
	.d(ddio_dataout_l_int),
	.q(wire_l_dffpipe_q));
	CL_lvds_rx1_cntr   bitslip_cntr
	( 
	.clock(fast_clock),
	.cnt_en(bitslip),
	.q(wire_bitslip_cntr_q));
	CL_lvds_rx1_mux   h_mux5a
	( 
	.data({cda_l_shiftreg4a[7], cda_h_shiftreg3a[6], cda_l_shiftreg4a[6], cda_h_shiftreg3a[5], cda_l_shiftreg4a[5], cda_h_shiftreg3a[4], cda_l_shiftreg4a[4], cda_h_shiftreg3a[3], cda_l_shiftreg4a[3], cda_h_shiftreg3a[2], cda_l_shiftreg4a[2], cda_h_shiftreg3a[1], cda_l_shiftreg4a[1], cda_h_shiftreg3a[0], cda_l_shiftreg4a[0], ddio_dataout_h[0]}),
	.result(wire_h_mux5a_result),
	.sel(bitslip_en));
	CL_lvds_rx1_mux   l_mux6a
	( 
	.data({cda_h_shiftreg3a[6], cda_l_shiftreg4a[6], cda_h_shiftreg3a[5], cda_l_shiftreg4a[5], cda_h_shiftreg3a[4], cda_l_shiftreg4a[4], cda_h_shiftreg3a[3], cda_l_shiftreg4a[3], cda_h_shiftreg3a[2], cda_l_shiftreg4a[2], cda_h_shiftreg3a[1], cda_l_shiftreg4a[1], cda_h_shiftreg3a[0], cda_l_shiftreg4a[0], ddio_dataout_h[0], ddio_dataout_l[0]}),
	...
	defparam
		...
		lvds_rx_pll.clk0_multiply_by = 8,
		lvds_rx_pll.clk0_phase_shift = "0",
		lvds_rx_pll.clk1_divide_by = 8,
		lvds_rx_pll.clk1_multiply_by = 8,
		lvds_rx_pll.clk1_phase_shift = "0",
		lvds_rx_pll.inclk0_input_frequency = 32000,
	...
	assign
		...
		rx_out = rx_reg,
		rx_out_wire = {l_shiftreg1a[7], h_shiftreg2a[7], l_shiftreg1a[6], h_shiftreg2a[6], l_shiftreg1a[5], h_shiftreg2a[5], l_shiftreg1a[4], h_shiftreg2a[4], l_shiftreg1a[3], h_shiftreg2a[3], l_shiftreg1a[2], h_shiftreg2a[2], l_shiftreg1a[1], h_shiftreg2a[1], l_shiftreg1a[0], h_shiftreg2a[0]},
		rx_outclock = slow_clock,
		slow_clock = wire_rx_outclock_buf_outclk;
endmodule //CL_lvds_rx1_lvds_rx
//VALID FILE
